...
herr_t Bayes_H5Gset_create_dataframe_group(hid_t root_id, const char *group_name, int col, int row, int col_str_len_max, const void *column, const void *index, const void *data)
{
	...
	hid_t axis1 = H5Dcreate(group_id, "axis1", H5T_NATIVE_ULLONG, row_space, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
	Bayes_H5Aset_dataset_global_attributes(axis1);
	Bayes_H5Aset_attribute_string(axis1, "kind", "datetime64");
	Bayes_H5Aset_attribute_string(axis1, "name", "time");
	Bayes_H5Aset_attribute_string(axis1, "freq", "N.");
	Bayes_H5Aset_attribute_string(axis1, "index_class", "datetime");
	Bayes_H5Aset_attribute_bitfield(axis1, "transposed", 1);
	status = H5Dwrite(axis1, H5T_NATIVE_ULLONG, H5S_ALL, H5S_ALL, H5P_DEFAULT, index);
